...
class alta_clientes(interna):
    ...
    def alta_clientes_cambiarDom(self, tipo, oParam):
        ...
        if len(aChecked) > 1:
            print("len mayor que 1")
            resul['status'] = -1
            resul['msg'] = "Ha selecionado mas de una dirección"
            return resul
        if aChecked[0] is None:
            print("len menor que 1")
            resul['status'] = -1
            resul['msg'] = "No ha selecionado ningúna dirección"
            return resul
